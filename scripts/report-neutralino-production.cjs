...
const HOST_NAV_METADATA = path.join(STAGE, 'neutralino-host-navigation-runtime.json');
...
function preferDistributionFile(files) { return files.find((file) => file.includes(`${path.sep}neutralino-dist${path.sep}`)) || files[0] || null; }
...
if (!binary || !extension || !fs.existsSync(HOST_NAV_METADATA)) throw new Error('Missing Neutralino production build output or provenance.');
const config = JSON.parse(fs.readFileSync(path.join(STAGE, 'neutralino.config.json'), 'utf8'));
const hostNavigationRuntime = JSON.parse(fs.readFileSync(HOST_NAV_METADATA, 'utf8'));
...
const report = {
  ...
  patchedRuntimeSha256: hostNavigationRuntime.binarySha256,
  embeddedBinary: { name: BINARY_NAME, bytes: binaryStats.size, sha256: sha256(binary) },
  externalLinkHelper: { name: EXTENSION_NAME, bytes: extensionStats.size, sha256: sha256(extension), bundledRuntimeDependency: false },
  ...
  phase1DeltaBytes: binaryStats.size - PHASE1_BINARY_BYTES,
  electronAppImageBaselineBytes: ELECTRON_APPIMAGE_BYTES,
  electronAppImageBaselineMiB: 92.6,
  productionArtifactContents: [BINARY_NAME, `extensions/${EXTENSION_NAME}`, 'provenance.json', 'SHA256SUMS'],
};
...
fs.writeFileSync(path.join(BUNDLE, 'provenance.json'), `${JSON.stringify(report, null, 2)}\n`);
const sums = [BINARY_NAME, `extensions/${EXTENSION_NAME}`, 'provenance.json'].map((relative) => `${sha256(path.join(BUNDLE, relative))}  ${relative}`).join('\n');
