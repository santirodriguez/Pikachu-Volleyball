...
function createControllerHarness(randomValues = []) {
  ...
  const deps = {
    ...
    gamePresentationModule: presentationModule,
  };
...
function capturePhysicsScenario() {
  const harness = loadPhysicsHarness([1, 2, 3, 4, 5, 6, 7, 8]);
  const physics = new harness.PikaPhysics(false, false);
  const inputs = [new harness.PikaUserInput(), new harness.PikaUserInput()];
  const checkpoints = [{ name: 'initial', physics: snapshotPhysics(physics) }];

  for (let frame = 0; frame < 3; frame += 1) {
    physics.runEngineForNextFrame(inputs);
  }
  checkpoints.push({ name: 'after-three-frames', physics: snapshotPhysics(physics) });

  const powerInput = new harness.PikaUserInput();
  powerInput.yDirection = -1;
  physics.ball.x = 100;
  physics.ball.y = 100;
  physics.ball.xVelocity = 0;
  physics.ball.yVelocity = 5;
  harness.processCollisionBetweenBallAndPlayer(
    physics.ball,
    100,
    powerInput,
    2
  );
  checkpoints.push({ name: 'power-hit', physics: snapshotPhysics(physics) });

  return { checkpoints, rngTrace: [...harness.rngTrace] };
}

function captureAiScenario() {
  const randomValues = Array.from({ length: 64 }, (_, index) => (index * 7919) % 32768);
  const harness = loadPhysicsHarness(randomValues);
  const physics = new harness.PikaPhysics(true, true);
  const inputs = [new harness.PikaUserInput(), new harness.PikaUserInput()];
  physics.player1.x = 100;
  physics.player2.x = 332;
  physics.ball.x = 180;
  physics.ball.y = 200;
  ...
  for (let frame = 0; frame < 6; frame += 1) {
    ...
    checkpoints.push({
      ...
      physics: snapshotPhysics(physics),
      rngCalls: harness.rngTrace.length,
    });
  }
...
function captureLifecycleScenario() {
  ...
  game.intro();
  checkpoints.push({ name: 'intro-first-frame', ...snapshotGame(game, rngTrace, recorder) });
  ...
  game.intro();
  checkpoints.push({ name: 'intro-timeout', ...snapshotGame(game, rngTrace, recorder) });
  ...
  setInput(game, 2);
  game.menu();
  checkpoints.push({ name: 'menu-inactivity', ...snapshotGame(game, rngTrace, recorder) });

  game.transitionTo(GAME_STATE_IDS.AFTER_MENU_SELECTION);
  game.frameCounter = 14;
  game.afterMenuSelection();
  checkpoints.push({ name: 'after-menu-boundary', ...snapshotGame(game, rngTrace, recorder) });

  game.frameCounter = 14;
  game.beforeStartOfNewGame();
  checkpoints.push({ name: 'before-game-boundary', ...snapshotGame(game, rngTrace, recorder) });

  game.frameCounter = 70;
  game.startOfNewGame();
  checkpoints.push({ name: 'start-game-boundary', ...snapshotGame(game, rngTrace, recorder) });
...
function captureScoringScenario() {
  ...
  game.round();
  checkpoints.push({ name: 'score-right', ...snapshotGame(game, rngTrace, recorder) });
  ...
  game.gameLoop();
  checkpoints.push({ name: 'slow-motion-fifth-tick', ...snapshotGame(game, rngTrace, recorder) });
...
function captureEndAndCommandsScenario() {
  ...
  game.round();
  checkpoints.push({ name: 'quick-rematch-before', ...snapshotGame(game, rngTrace, recorder) });
  game.round();
  checkpoints.push({ name: 'quick-rematch-trigger', ...snapshotGame(game, rngTrace, recorder) });
  ...
  game.round();
  checkpoints.push({ name: 'practice-reset', ...snapshotGame(game, rngTrace, recorder) });
  ...
  game.restart();
  checkpoints.push({ name: 'restart', ...snapshotGame(game, rngTrace, recorder) });

  return { checkpoints };
}

function createReferenceTraces() {
  return {
    schemaVersion: 1,
    source: 'v3-restart@60f978ec77e1a2c8adf5d56ae14102dccf41d1a9',
    physics: capturePhysicsScenario(),
    ai: captureAiScenario(),
...
if (require.main === module) {
  process.stdout.write(`${JSON.stringify(createReferenceTraces(), null, 2)}\n`);
}
