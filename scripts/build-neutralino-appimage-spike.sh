...
build_bundled() {
  ...
  local appdir="$work/bundled.AppDir" webkit_lib wayland_client_lib gbm_lib egl_lib webkit_process_dir scanner plugin
  ...
  gbm_lib="$(ldconfig -p | awk '/libgbm.so.1/ && !found {value=$NF; found=1} END {if (found) print value}')"
  [[ -f "$gbm_lib" ]] || { echo "Bundled candidate builder is missing GBM runtime" >&2; exit 1; }
  egl_lib="$(ldconfig -p | awk '/libEGL.so.1/ && !found {value=$NF; found=1} END {if (found) print value}')"
  [[ -f "$egl_lib" ]] || { echo "Bundled candidate builder is missing EGL runtime" >&2; exit 1; }
  ...
  local -a deploy_args=(--appdir "$appdir" --library "$webkit_lib" --library "$wayland_client_lib" --library "$gbm_lib" --library "$egl_lib")
  ...
  cp -L --remove-destination "$gbm_lib" "$appdir/usr/lib/libgbm.so.1"
  [[ -f "$appdir/usr/lib/libgbm.so.1" ]] || { echo "Bundled candidate is missing libgbm.so.1" >&2; exit 1; }
  cp -L --remove-destination "$egl_lib" "$appdir/usr/lib/libEGL.so.1"
  [[ -f "$appdir/usr/lib/libEGL.so.1" ]] || { echo "Bundled candidate is missing libEGL.so.1" >&2; exit 1; }
  for plugin in "${gst_plugins[@]}"; do
  ...
  dpkg-query -W -f='${Package}\t${Version}\t${Architecture}\n' \
    libwebkit2gtk-4.1-0 libjavascriptcoregtk-4.1-0 libgtk-3-0 libglib2.0-0 libwayland-client0 libgbm1 libegl1 \
